  ...
  geometric3dShapes.prototype.mapPlain = function(geometry,s, scaler){
    for ( var i = 0, l = s.geometry.vertices.length; i < l; i ++ ) {
      if(geometry.vertices.length > i){
         geometry.vertices[ i ].x = scaler*s.geometry.vertices[ i ].x;
         geometry.vertices[ i ].y = scaler*s.geometry.vertices[ i ].y;
         geometry.vertices[ i ].z = scaler*s.geometry.vertices[ i ].z;
      }
    }
  }
  ...
  geometric3dShapes.prototype.getColorOfVertices_p = function(geometry, scaler){
    ...
    geometric3dShapes.prototype.calculateColor_p = function(pos, scaler, rawDiff){
      ...
      var color2d = this.postionColor(this.size, [pos.x + rawDiff,
                                                  pos.z + rawDiff]);         
      color.setHSL(color2d[0],color2d[1] ,value );
      return color;
    }

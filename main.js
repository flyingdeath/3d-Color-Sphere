...
//                                                                                               x:300,y:300,z:0});
  ...
  var mouseMoveCallBack = function(pos,element){
    ...
    element.hud.updatePosition(element.camera, element.controls.getTarget());
  /*
  ...
  var mouseUpCallBack = function(ray,element){
   ...
   //   var vector = origin.sub(origin, direction);
      ...
      point = origin.subVectors(point, origin);
  ...
  //  if(new helperClass().getSelectValue('SystemType_XColor') == "Geometric"){
      var polyColors = element.geo.getColorOfVertices(element.poly.update(null, null, element.current));
